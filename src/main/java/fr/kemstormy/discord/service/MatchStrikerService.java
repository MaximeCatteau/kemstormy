...
@Service
public class MatchStrikerService {
    @Autowired
    ...
    public List<StrikerLadderResource> getStrikersLadder(Long leagueId) {
        ...
        List<StrikerLadderResource> strikerLadderResources = new ArrayList<>();
        ...
        try {
            for (Tuple t : tuples) {
                ...
                strikerLadderResources.add(res);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    
        return strikerLadderResources;
    }

    public EmbedBuilder createEmbedStrikersLadder(List<StrikerLadderResource> strikerLadder, Long leagueId) {
        League league = this.leagueRepository.findById(leagueId).orElseThrow();
        EmbedBuilder embedBuilder = new EmbedBuilder();

        BufferedImage image;
        try {
            image = this.createStrikersLadderImmage(strikerLadder);
    ...
    private BufferedImage createStrikersLadderImmage(List<StrikerLadderResource> strikerLadder) throws FontFormatException, IOException {
        ...
        String outputFilePath = "strikers_ladder.png"; // Output file path
        ...
        drawPicture(graphics, parameter1, parameter2, fillColor, strikerLadder);
    ...
    private void drawPicture(Graphics graphics, int parameter1, int parameter2, Color fillColor, List<StrikerLadderResource> ladder) throws FontFormatException, IOException {
        // Register font
        ...
        graphics.drawString("Buts marqués", 1060, 50);
        ...
        for (StrikerLadderResource l : ladder) {
            ...
            graphics.drawString("" + l.getScoredGoals(), 1060, heightThreshold);
